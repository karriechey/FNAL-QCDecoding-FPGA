...
#
# A distance-scaling study with exactly one rung (10M). The only flags that differ from
# the established d=5, r=5 teacher runs are --d and --rounds; everything else is copied
# from eaf_run_rcnn_ladder_r5.sh unchanged, because a hyperparameter that moves with
# distance would confound the comparison the experiment exists to make.
#
# Timing mode. The d=9 per-epoch cost is unmeasured on GPU. The RCNN has 49 kernels at
# d=9 against 9 at d=5, and 9 rounds against 5, so expect near 10x the d=5 teacher's
# ~4.7 h/seed -- roughly 47 h/seed, 140 h for three seeds. That is arithmetic. Run one
# epoch first and read the real number:
#
...
#
# Reporting partition is the validation block [10.0M, 10.2M). --seal-test leaves the test
...
# d=9 is open -- activation memory grows ~10x with the kernel and round counts, and a
# local batch-10,000 probe was OOM-killed on a 16 GB host. Run probe_batch_d9.py on this
...
echo "  validation [$VAL_START, $((VAL_START+VAL_N)))   test left sealed"
echo "  out=$OUTROOT"
[ "$EPOCHS" = "1" ] && echo "  *** timing mode: 1 epoch, diagnostic only ***"
echo
