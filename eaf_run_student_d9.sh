...
#
# Hard label, alpha=1.0. The d=5, r=5, 10M reference these runs are compared against is
# out_student_r5_hard (GRU 0.08145, MLP 0.09756 on the validation block), so the d=9 runs
# have to match it -- a hard-label d=5 point against a distilled d=9 point would make the
# distance contrast meaningless. train_student.py writes alpha and temperature into every
# result row, so the CSV records the regime.
#
...
#
# Sizes are the d=5 values, held fixed across the distance change:
#   MLP hidden=(209,209)   69,389 params at d=5  ->  194,789 at d=9  (2.81x)
#   GRU units=140          69,441 params at d=5  ->   92,961 at d=9  (1.34x)
# The parameter growth is itself a result. Exp 10 asks how the existing architecture
# families scale; forcing equal parameter counts is a different study, out of scope.
#
...
echo "  mlp hidden=($MLP_HIDDEN)   gru units=$GRU_UNITS   (d=5 sizes, held fixed)"
...
for arch in $ARCHS; do
  ...
  for s in $SEEDS; do
    i=$((i+1))
    # The learning rate is in the tag so a diagnostic at another rate writes to its own
    # filenames.
